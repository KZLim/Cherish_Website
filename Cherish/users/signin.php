<html>
    ...
    <body>
        <div class="register-container">
            <div class="inner-container" style="background-color: white; padding-top: 70px;">
                ...
                <form action="../cherish_api/users/api_signin.php" method="POST">
    ...
    <?php
        if(!empty($_GET['response'])){
            $alertCondition = $_GET['response'];
            if($alertCondition == "acclocked"){
                echo"<script>
                        alertify.alert('Sign In Disabled','Your account has been locked. Contact users suppport now!');
                    </script>";
            }
            else if($alertCondition == "invcred"){
                echo"<script>
                        alertify.alert('Account Action','Invalid Credential Verification. Try Again.');
                    </script>";
            }
            else if($alertCondition == "anp"){
                echo"<script>
                        alertify.alert('Password Reset','Action Not Performed. Try Again.');
                    </script>";
            }
        }
    ?>
