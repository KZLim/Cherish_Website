...
if( 
    ...
    $ngo->activityStatus = 'updated';
    $ngo->ouid = $_SESSION['identifier'];
    $ngo->activityID = $_POST['activityIdParam'];

    //Call the function update activity. The function is define in the obj file.
    if($ngo->updateActivity()){
        //make a redirection here when successfully created the profile.
        header("Location: ../../main/volunteerActivity.php?activityid=$ngo->activityID&action=updateSuccess");
    }
    //Unable to update activity
    else{
        header("Location: ../../main/volunteerActivity.php?activityid=$ngo->activityID&action=uaanp");      
    }
}
// Data missing
else{
