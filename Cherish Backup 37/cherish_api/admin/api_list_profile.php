...
if( 
    ...
    !empty($_GET['targetOUID'])
   
){
    //if none data are missing, the data will be sanitize
    $providedOuidReady = htmlspecialchars(strip_tags($_GET['targetOUID']));

    // set the value to the object properties to be used 
    $ngo->status = "listed";
    $campaign->campaignStatus = "listed";
    $activity->activityStatus = "listed";
    ...
    if($ngo->updateNgoStatus()){
        if($campaign->updateAllCampaignStatus()){
            if($activity->updateAllActivityStatus()){
                header("Location: ../../admin/ngoManagement.php?actionDone=listed");
            }
        }
    }
    else{
